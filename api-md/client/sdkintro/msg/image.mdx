...
图片消息是 SDK 内置消息类型，枚举对应 _[MessageType.IMAGE](../../enum/web#message)_
...
图片消息（ImageMessageContent）是 SDK 内置的消息类型，对应的 contentType 是 "jg:img"。
...
| height       | number      | 图片高度         | 1.0.0 |
| width        | number      | 图片宽度         | 1.0.0 |
| size         | number     | 图片大小，单位 `Byte` | 1.0.0 |
| extra | string | 扩展字段 | 1.0.0 |
