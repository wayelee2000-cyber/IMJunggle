...
文件消息是 SDK 内置消息类型，枚举对应 _[MessageType.FILE](../../enum/web#message)_
...
文件消息（FileMessageContent）是 SDK 内置的消息类型，对应的 contentType 是 "jg:file"。
...
| size   | number | 文件大小，单位 `Byte` | 1.0.0 |
| type   | string  | 文件类型            | 1.0.0 |
| extra | string | 扩展字段 | 1.0.0 |
