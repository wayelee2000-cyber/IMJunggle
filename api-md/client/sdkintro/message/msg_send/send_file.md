...
| message.content.file           | File    | 是     |        | 发送 File 对象，通过 `<input type="file">` 获取               | 1.0.0    |
| message.content.name           | String  | 是     |        | 自定义文件名称，透传给对端                     | 1.0.0    |
| message.content.type           | String  | 是     |        | 文件类型，透传给对端，用来自定义文件类型并展示 UI            | 1.0.0    |
...
由于发送消息是异步的，调用 `sendFileMessage` 会同步返回 `message` 对象，此时可优先向页面展示消息，

并用 `message.clientMsgNo` 做唯一标识，`callback` 触发后可根据 `clientMsgNo` 做消息状态更新，如上传进度。

SDK 内置文件上传功能，只需将本地文件路径赋值给 `FileMessage.localPath`，SDK 会自动上传文件并返回文件 URL。
