...
Developers only need to pass in the File object; the send video method will automatically extract the first frame of the video as the cover image. For more attributes, please refer to [Video Message](../../msg/video.md).
...
|-----------|----------|-------------------------------------------------------------------------------|--------|
| message | Object | After successful sending, a message object with `messageId` and `sentTime` is returned. See the [message object](../../msg/message.md) | 1.0.0 |
...
const videoMessage = {
  conversation: conversation,
  content: {
    contentType: 'jg:video',
    localPath: '/path/to/video.mp4',
    snapshotLocalPath: '/path/to/snapshot.jpg',
    width: 600,
    height: 400,
    duration: 10,
    size: 1024
...
const callback = (message: any, errorCode: number, progress?: number) => {
  if (errorCode === 0) {
    console.log('sendMediaMessage success, messageId is ' + message.messageId);
  } else if (errorCode === 100) {
    console.log('sendMediaMessage onProgress, progress is ' + progress);
  } else {
    console.log('sendMediaMessage error, errorCode is ' + errorCode.toString());
  }
};

JuggleIM.sendMediaMessage(videoMessage, callback).then((message) => {
...
videoMessage.localPath = '/Applications/xxx/xxx/video.mp4';
videoMessage.snapshotLocalPath = '/Applications/xxx/xxx/snapshot.jpg';
videoMessage.height = 400;
videoMessage.width = 600;
...
Message mediaMessage = await JuggleIm.instance.sendMediaMessage(videoMessage, conversation, callback, progressCallback);
```
